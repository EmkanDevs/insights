frappe.pages['customer-insights'].on_page_load = function(wrapper) {
	...
	if (frappe.route_options && frappe.route_options.customer) {
		// Take the first customer from the array
		customerParam = frappe.route_options.customer[0];
		// Clear route_options after reading so it doesn't affect future navigations
	...
	setTimeout(() => {
		...
		if (customerField && customerField.df.fieldtype === 'MultiSelectList') {
			// For MultiSelectList, assign values and refresh
			customerField.set_value([customerParam]); // works for array in MultiSelectList
			customerField.refresh();
		}
	}, 500); // you can reduce timeout if rendering is quick
...
frappe.pages['customer-insights'].on_page_show = function(wrapper) {
    if (frappe.route_options && frappe.route_options.customer) {
        let customerParam = frappe.route_options.customer[0];
        frappe.route_options = null;

        const customerField = wrapper.form.get_field('customer');
        if (customerField && customerField.df.fieldtype === 'MultiSelectList') {
            customerField.set_value([customerParam]);
            customerField.refresh();
        }

        filters.customer = [customerParam];
