frappe.pages['supplier-insights'].on_page_load = function(wrapper) {
	...
	if (frappe.route_options && frappe.route_options.supplier) {
		// Take the first supplier from the array
    ...
    // Set supplier field value if coming from URL parameter
	setTimeout(() => {
		...
		if (supplierField && supplierField.df.fieldtype === 'MultiSelectList') {
			...
			supplierField.set_value([supplierParam]); // works for array in MultiSelectList
    ...
    // Initial fetch with filters (including supplier from URL if present)
    fetch_supplier_insights();
};
